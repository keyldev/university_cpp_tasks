...
string temp_str;
bool exit(string maze[], int str, int col)
{
	maze[str][col] = '.';
	...
	{
		if (isalpha(maze[str - 1][col])) { temp_str = temp_str + ' ' + maze[str - 1][col]; maze[str - 1][col]; maze[str - 1][col] = '*'; }
		maze[str][col] = '@'; col--; maze[str][col] = '@';
	}
	else if (maze[str - 1][col] == '.')
	{
...
int main()
{
    ...
    do
    {
        cout << endl << "Введите стартовые координаты через пробел(№ колонки, № столбца): ";
        cin >> col >> str;
		if (maze[str][col] == '#' )
